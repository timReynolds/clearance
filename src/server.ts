...
const server = createServer((request, response) => {
  ...
  if (request.url?.startsWith("/api/me") === true) {
    handleAsyncRequest(response, () => handleMeApiRequest(request, response));
    return;
  }

  if (request.url?.startsWith("/auth/github/start") === true) {
    handleAsyncRequest(response, () => handleGithubOAuthStart(request, response));
    return;
  }

  if (request.url?.startsWith("/auth/github/callback") === true) {
    handleAsyncRequest(response, () => handleGithubOAuthCallback(request, response));
    return;
  }

  if (request.url?.startsWith("/auth/logout") === true) {
    handleAsyncRequest(response, () => handleLogout(request, response));
    return;
  }

  if (request.url?.startsWith("/api/review/") === true) {
    handleAsyncRequest(response, () => handleReviewApiRequest(request, response));
    return;
  }
  ...
  if (
    ...
    handleAsyncRequest(response, () => servePublicAsset(request, response));
    return;
  }
...
function handleAsyncRequest(response: ServerResponse, handler: () => Promise<void>): void {
  void handler().catch((error) => {
    console.error(error, "request processing failed");
    if (!response.headersSent) {
      writeJson(response, 500, {
        error: getErrorMessage(error, "request processing failed"),
      });
      return;
    }
